...
## D43 — The stinger: an ident, not a loading screen — decided

Once per visit, before the screen becomes a workspace, a wave of colour crosses the warm wallpaper the way pigment opens across a bobtail squid's mantle, the mark and wordmark resolve out of it on a wallpaper free of windows, and the windows cut in as the ident goes. About 1.3 seconds, never more; a test holds the sequence under 1.4. It is defined as data like every other move: one sequence file (`stinger.json`), two small verbs in the runner (`sweep_x`, `settle`; settle is opacity only so centred windows keep their own transform), a `stung` flag in state. It never delays an agent: WebMCP registration starts before it plays, and the first move cuts it through a one-shot hook in dispatch. Any pointer, key, or wheel input cuts it too. Under reduced motion it never shows. It is not a loading screen; reading the ice has its own motion. It is the ident: you're watching a broadcast, and the name means something before a word is read.

## D44 — The voxel Sepiola is an illustration, used only on the social card — decided

A voxel rendering of the mascot (generated with ChatGPT for another project, in this project's palette) is warmer than the crest at large sizes and unreadable below about 48 pixels, where the crest still works. So the flat crest stays the mark everywhere on the page: favicon, menubar, window titles, ident. The voxel version appears in exactly one place, the 1280×640 social card (`brand/social-card.png`, rendered by `npm run brand:card`), which serves as the GitHub social preview and the page's Open Graph image. `brand/` holds source assets that never enter the page. The credits line in About says where the illustration came from.

## D45 — The first thirty seconds — decided

A review from a stranger's seat found the product strong and the arrival weak: the welcome led with a name the visitor did not know, builders had no door, the sample dumped five windows with nothing pointing at the best moments, the grey lamp read as a fault, and on a phone only the console showed. Five changes. (1) **Watch the pen**: the sample plays a scripted run of real moves (circle, run it back, split, cut to panel) with a caption each, ~15 s, through `run()` so the Talkback fills and every rule holds; skip, Escape, or any pointer ends it. It is a viewer affordance, not a grammar verb, and it is the producer idea of a roll used as a demo. (2) **Two doors**: the welcome says what and why in plain words, then "I play fantasy hockey" and "I build with agents". (3) **Connect your agent**: an About section with the Chrome flag, the Codex browser, and a copyable `claude mcp add` command for the stateless `/mcp`; the lamp says "no agent here". (4) **Phone layout** below 760 px: one column, overlays full-width, no dock, lamps as dots. (5) **Plumbing folded**: the move list and the source line sit behind a disclosure. Captions and doors are copy; the screen still composes nothing.

## D46 — Plain words, and windows that stay under the menubar — decided

A newcomer meets three vocabularies at once: fantasy hockey (start/sit, stream, back-to-back, games in hand, IR), the broadcast (run it back, talkback, telestrator), and the technology (agent, MCP, WebMCP). About gains "The words, in plain language", a glossary reachable from the welcome, the menu, and `/#words`; the panel's call tags and the games-in-hand title explain themselves on hover; the quick start is rewritten as six plain steps that assume none of the vocabulary. Separately: window stacking used the raw raise counter as z-index, which grows every time a window is raised and eventually climbed over the menubar's menus. Windows are now ranked among themselves on every render (10 + rank), and the menubar sits at 90, above windows, dock, caption, and skater menu.

## D47 — Goalie streams on public data; each night rated from the player's side — decided

Of the three concept tools, Goalie Streaming is the one buildable honestly now. CHIRP's `analyze_goalie_streams` ranks every goalie by games in the window, each opponent's attack (standings goals-for, ranked), and last season's start share, with GAA, save % and a stream score whose formula is in the output. It states what it cannot know: starters are not announced in public data, and before opening night the standings are last season's final table. The read gains optional `skater.nights`: per day the opponent, home or away, and a difficulty in this player's terms, the opponent's attack for a goalie and its defence for a skater, so the same game can be soft for one and hard for another. The replay draws the opponent on each game tile and a green or red edge from the analyst's number, with a key. Team Buy-In waits for ten to fifteen games of ice time; Injury Impact stays a concept until there is an honest injury source.

## D48 — The draft board: a second read, an eighth move, the same grammar — decided

Drafts happen now, and a draft board is as visual as a lineup. It is also a different read, so it gets its own contract (`contracts/board.schema.json`: tiers per position, one note per prospect, `taken`, where each position thins out, `not_included`), its own fixture (a real `draft_kit` board from 2026-09-22), and its own analyst route (CHIRP `POST /board`, stateless, drafted players in the request). The screen crosses nothing off and ranks nothing: the analyst resolves the drafted names and returns them marked, with unresolved lines as notes. One new move, `cue_board`, puts up the board; it loads a subject like `cue_roster`, it does not think. The existing moves transfer: `circle` on a prospect pins the analyst's note under his row, `replay` shows his card, `split` shows two cards, and a split across the rink and the board is refused. The grammar is now eight moves, and this is the first evidence that the telestrator grammar carries across reads, which the paper claims. The page's contract checker gained `anyOf` and validates any schema, so live boards are checked like live reads.

## D49 — The pick is the analyst's; the board only marks it — decided

A board tells you who is left; at the table the question is who to take. CHIRP already answers it (`chirp_draft_pick`: the best producers left, weighted to the positions you still need and your playoff-week schedule), so the answer arrives in the board read rather than in a new move. `cue_board` gains `mine_text`, your own picks; given it, `POST /board` counts them as drafted, runs `chirp_draft_pick` against the draft so far, and adds an optional `pick` to the board: the pick on the clock, the positions you still need, the analyst's one line, and its top three with a reason each and whether each sits in the columns. The screen draws a strip above the tiers and rings those cards; the numbers come from the list's order through CSS counters and a `data-mark` attribute, so the screen writes no digit of its own and ranks nothing. A pick that falls below the columns is named in the strip but not clickable, since the moves act on prospects the board holds. A new pick scrolls the board back to its top once, because the answer lands above the boxes you just typed in. Still eight moves. Left out: playoff weeks on the page (the pen and the HTTP route already take them).

## D50 — Draft boxes survive a reload, in this browser only — decided
...
## D51 — Rank for the league you are in — decided
...
Built 2026-09-22 against a real 12-team Yahoo head-to-head league (G, A, +/-, PIM, PPP, SHP, GWG, SOG, HIT, BLK; W, GAA, SV, SV%, SHO). Two calibrations came from reading that board. A player's value is the *sum* of his category scores, not the mean: goalie categories move together while a skater's pull apart, so a mean put four goalies in the top seven, and a sum (each category counting once, as in a matchup) puts the first goalie 37th, early in round four. Each category is capped at ±3, because a rare stat (shorthanded points) otherwise decides a player alone. Building it also found that traded players carried only their last club's games (81 players; Quinn Hughes 48 games instead of 74); lines are now whole seasons from the NHL's league-wide stats service. The board's button became "Update board", since categories alone are reason to redraw it. Checking the league's playoff weeks against Yahoo's settings found one more: CHIRP counted fantasy week 1 from the Monday of the opener's week, so with a Tuesday opener every playoff window was a week early. Week 1 now opens with the season and folds a short opening week in, as Yahoo counts it (2026-27 weeks 25–27: March 22 – April 11).
